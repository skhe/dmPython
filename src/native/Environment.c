...
)
{
	...
	{
        aq_sprintf(error_buf, 4096, "[CODE:%d]%s", errorCode, errorText);
...
)
{
	...
	{
	case PG_UTF8:
        aq_sprintf(code_name, 128, "%s", "UTF8");		
		break;

	case PG_GBK:
        aq_sprintf(code_name, 128, "%s", "GBK");		
		break;

	case PG_BIG5:
        aq_sprintf(code_name, 128, "%s", "BIG5");		
		break;

	case PG_ISO_8859_9:
        aq_sprintf(code_name, 128, "%s", "ISO_8859_9");		
		break;

	case PG_EUC_JP:
        aq_sprintf(code_name, 128, "%s", "EUC_JP");		
		break;

	case PG_EUC_KR:
        aq_sprintf(code_name, 128, "%s", "EUC_KR");				
		break;

	case PG_KOI8R:
        aq_sprintf(code_name, 128, "%s", "KOI8-R");		
		break;

	case PG_ISO_8859_1:
        aq_sprintf(code_name, 128, "%s", "ISO_8859_1");		
		break;

	case PG_SQL_ASCII:
        aq_sprintf(code_name, 128, "%s", "ASCII");
		break;

	case PG_GB18030:
        aq_sprintf(code_name, 128, "%s", "GB18030");		
		break;	

    case PG_ISO_8859_11:
        aq_sprintf(code_name, 128, "%s", "ISO_8859_11");        
        break;
...
Environment_New() 
{
    ...
    aq_sprintf(env->encoding, "%s", "gb18030"); 
    
	// 申请环境句柄
	rt	= dpi_alloc_env(&env->handle);
    if (!DSQL_SUCCEEDED(rt))
    {
        aq_sprintf(context, 512, "Environment_New():alloc environment handle");
        goto fun_end;
    }	
    ...
    if (!DSQL_SUCCEEDED(rt))
    {
        aq_sprintf(context, 512, "Environment_New():get local_code");
        goto fun_end;
    }       	
...
fun_end:
    if (DSQL_SUCCEEDED(rt))
    {
        ...
        {
            aq_sprintf(context_tmp, 512, "Environment_New: Invalid local code [%d] has been got.", env->local_code);
