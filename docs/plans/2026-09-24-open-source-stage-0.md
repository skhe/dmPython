...
更新于 2026-09-24。目标是先确定包身份、支持边界、额外引入的 Go 驱动的许可状态和可重复的真实数据库测试环境，再推进自动发布到 PyPI。此阶段不创建公开 PyPI 版本。
...
| 隔离测试库 | 本机已就绪；CI 连通待完成 | 维护者 | [OrbStack ARM 实例首轮基线](../test-results/2026-09-25-orb-arm-baseline.md) 64 项通过、无残留表；仍需 CI 可达性和持续有效的测试许可 |
...
3. 用 `DM_TEST_HOST`、`DM_TEST_PORT`、`DM_TEST_USER`、`DM_TEST_PASSWORD` 配置连接。测试已改为必须显式提供这四项，子进程测试也沿用相同配置。PR 在未配置数据库凭据时可跳过集成测试；定时、手动完整回归和发布标签使用 `--require-dm`，缺配置或断连会失败。跳过不等于通过。
...
阶段 0 完成需要同时满足：包名复核、平台元数据一致、源码包内容检查通过、内置 Go 驱动的许可状态或替代方案明确、隔离测试库及非管理员账号就绪、完整真实数据库回归有记录。当前本机隔离库和首轮完整回归已有记录；Go 驱动许可、CI 数据库连通及持续有效的测试许可仍待解决，因此阶段 0 不能标为完成。
