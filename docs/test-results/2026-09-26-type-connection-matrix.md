...
`mpp_login`、`rwseparate` 和 `rwseparate_percent` 原先被桥接层静默忽略；现在校验取值并传到 Go 驱动，连接对象读回与传入一致。单机 DM8 已验证 MPP LOCAL 可以建连并查询；Go 驱动测试验证了 MPP LOCAL、读写分离模式 4 和 80% 路由比例的解析。当前没有主备或 MPP 集群，无法据此确认真实读写路由及 MPP 集群行为。非空的 SSL 和 UKey 参数目前会明确报错，防止请求安全连接却得到普通连接；证书与硬件认证能力仍待实现和验证。
